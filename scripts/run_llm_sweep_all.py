...
def run_for_game(game, run_root, tag: str, k_values: list[int], n_runs: int, mode: str) -> None:
...
def main() -> None:
    ...
    run = make_run_dir(tag="workflow_sweep_all", create_standard_dirs=False)
    run_root = run.root
    ...
    write_run_meta(
        run_root / "run_meta.json",
        {
            ...
            "k_values": k_values,
            "n_runs": n_runs,
            "agent_backend": "workflow_baseline",
            "notes": "Per-game outputs under run_root/<game>/",
        },
    )

    run_for_game(PrisonersDilemma(), run_root, "prisoners_dilemma", k_values, n_runs, mode)
    run_for_game(StagHunt(), run_root, "stag_hunt", k_values, n_runs, mode)
    run_for_game(BattleOfSexes(), run_root, "battle_of_sexes", k_values, n_runs, mode)
    run_for_game(UltimatumGame(), run_root, "ultimatum", k_values, n_runs, mode)
